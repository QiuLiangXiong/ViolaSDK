...
static void print_css_node_rec(
  ...
  if (options & CSS_PRINT_CHILDREN && node->children_count(node->context) > 0) {
    printf("children: [\n");
    for (int i = 0; i < node->children_count(node->context); ++i) {
...
static void layoutNodeImpl(css_node_t *node, float parentMaxWidth, float parentMaxHeight, css_direction_t parentDirection) {
  ...
  int childCount = node->children_count(node->context);
        ...
        if (child->style.position_type == CSS_POSITION_RELATIVE) {
          ...
          if (alignItem == CSS_ALIGN_STRETCH) {
            ...
            if (!isStyleDimDefined(child, crossAxis)) {
              ...
              if (dimCrossAxis != child->layout.dimensions[dim[crossAxis]] && child->children_count(child->context) > 0) {
...
void layoutNode(css_node_t *node, float parentMaxWidth, float parentMaxHeight, css_direction_t parentDirection) {
  ...
  if (skipLayout) {
    ...
    for (int i = 0, childCount = node->children_count(node->context); i < childCount; i++) {
